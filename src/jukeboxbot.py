...
def adminonly(func):
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if update.message.chat.type == "private":
            ...
            for member in await context.bot.get_chat_administrators(
        ...
        message = await context.bot.send_message(
            chat_id=update.effective_chat.id, text=jukeboxtexts.you_are_not_admin
        )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # send the message
    message = await context.bot.send_message(
        chat_id=update.effective_chat.id, text=jukeboxtexts.help
    )
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    message = await context.bot.send_message(
        chat_id=update.effective_chat.id, text=statsText
    )
...
async def balance(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
                ...
                [
                    [
                        InlineKeyboardButton(
                            f"Take me there", url=f"https://t.me/{bot_me.username}"
                        )
                    ]
                ]
            ),
    ...
    user = await userhelper.get_or_create_user(
    ...
    message = await context.bot.send_message(
        chat_id=update.effective_chat.id, text=f"Your balance is {balance} sats."
    )
...
""",
        ...
        if sps.client_id is None:
            await context.bot.send_message(
                chat_id=update.effective_user.id, text=jukeboxtexts.no_client_id_set
            )
        else:
            await context.bot.send_message(
                chat_id=update.effective_user.id,
        ...
        if sps.client_secret is None:
            await context.bot.send_message(
                chat_id=update.effective_user.id, text=jukeboxtexts.no_client_secret_set
            )
        else:
            await context.bot.send_message(
                chat_id=update.effective_user.id, text=jukeboxtexts.client_secret_set
            )
    ...
    if sps.client_id is not None and sps.client_secret is not None:
        ...
        auth_manager = await spotifyhelper.init_auth_manager(
        ...
        message = await context.bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            jukeboxtexts.button_to_private_chat,
        ...
        state = base64.b64encode(
            f"{update.effective_chat.id}:{update.effective_user.id}".encode("ascii")
        ).decode("ascii")
...
async def queue(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        message = await context.bot.send_message(
            chat_id=update.effective_chat.id, text="Failed to retrieve queue"
        )
        context.job_queue.run_once(
            delete_message,
            settings.delete_message_timeout_medium,
    ...
    for i in range(min(10, len(result["queue"]))):
        item = result["queue"][i]
        text += " {count}. {title}\n".format(
            count=(i + 1), title=spotifyhelper.get_track_title(item)
        )
...
async def spotify_settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    result = re.search(
        "/(setclientid|setclientsecret)\s+([a-z0-9]+)\s*$", update.message.text
    )
    if result is None:
        message = await context.bot.send_message(
            chat_id=update.effective_chat.id, text=f"Incorrect usage. "
        )
        return
...
async def fund(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        f"Fund sats",
                        url=f"https://{settings.domain}/jukebox/fund?command={telegramhelper.add_command(TelegramCommand(update.effective_user.id,'FUND'))}",
                    )
                ]
            ]
        ),
    ...
    context.job_queue.run_once(
        delete_message, settings.delete_message_timeout_long, data={"message": message}
    )
...
async def pay(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if payment_result["result"]:
        await context.bot.send_message(
            chat_id=update.effective_user.id, text="Payment succes."
        )
...
async def search(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(searchstr) > 1:
        ...
        message = await context.bot.send_message(
            chat_id=update.effective_chat.id, text=jukeboxtexts.add_command_help
        )
        context.job_queue.run_once(
            delete_message,
            settings.delete_message_timeout_medium,
                    ...
                    [
                        InlineKeyboardButton(
                            ...
                            callback_data=telegramhelper.add_command(
                                TelegramCommand(
                                    0, telegramhelper.playrandom, playlistid
                                )
                            ),
                        )
                    ]
    ...
    while numtries > 0:
        try:
            ...
            if numtries == 0:
                ...
                logging.error(
                    "Spotify returned and exception, not returning search result"
                )
                message = await context.bot.send_message(
                    chat_id=update.effective_chat.id,
    ...
    if len(result["tracks"]["items"]) > 0:
        ...
        button_list.append(
            [
                InlineKeyboardButton(
                    ...
                    callback_data=telegramhelper.add_command(
                        TelegramCommand(
                            update.effective_user.id, telegramhelper.cancel, None
                        )
                    ),
                )
            ]
        ...
        message = await context.bot.send_message(
            chat_id=update.effective_chat.id, text=f"No results for '{searchstr}'"
        )
        context.job_queue.run_once(
            delete_message,
            settings.delete_message_timeout_short,
...
async def dj(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    sender = await userhelper.get_or_create_user(
        update.effective_user.id, update.effective_user.username
    )
    ...
    recipient = await userhelper.get_or_create_user(
        update.message.reply_to_message.from_user.id,
        update.message.reply_to_message.from_user.username,
    )
    invoice = await invoicehelper.create_invoice(
        recipient, amount, f"@{sender.username} thinks you're a DJ!"
    )
    ...
    if result["result"] == True:
        ...
        message = await context.bot.send_message(
            chat_id=update.effective_chat.id, text=f"Payment failed. Sorry."
        )
        context.job_queue.run_once(
            delete_message,
            settings.delete_message_timeout_short,
...
async def callback_paid_invoice(invoice: Invoice):
    ...
    try:
        logging.info(
            f"Trying to delete chat_id {invoice.chat_id}, messageid {invoice.message_id}"
        )
    ...
    try:
        ...
        logging.error(
            "Could not  send message to the group that track was added to the queue"
        )

    try:
        async with aiomqtt.Client("localhost") as client:
            await client.publish(
                f"{invoice.chat_id}/added_to_queue", payload=invoice.title
            )
    except:
    ...
    donation_invoice = await invoicehelper.create_invoice(
        jukeboxbot, donation_amount, "donation to the bot"
    )
...
async def web(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    filename = os.path.join(
        settings.qrcode_path, f"web_url_{update.effective_chat.id}.png"
    )
...
async def callback_spotify(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
            ...
            if chat_id in now_playing_message:
                ...
                if prev_title != title:
                    ...
                    try:
                        async with aiomqtt.Client("localhost") as client:
                            await client.publish(
                                f"{chat_id}/now_playing", payload=title
                            )
                    except:
        ...
        context.job_queue.run_once(
            callback_spotify, interval, job_kwargs={"misfire_grace_time": None}
        )
...
async def callback_button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    amount_to_pay = int(
        (await spotifyhelper.get_price(update.effective_chat.id))
        * len(spotify_uri_list)
    )
    ...
    if payment_required == False:
        ...
        for uri in spotify_uri_list:
            ...
            try:
                async with aiomqtt.Client("localhost") as client:
                    await client.publish(
                        f"{update.effective_chat.id}/added_to_queue", payload=tracktitle
                    )
            except:
                logging.error("Exception when publishing queue add to mqtt")
                pass
    ...
    invoice = await invoicehelper.create_invoice(
        recipient, amount_to_pay, invoice_title
    )
    ...
    if payment_result["result"] == True:
        ...
        try:
            async with aiomqtt.Client("localhost") as client:
                await client.publish(
                    f"{update.effective_chat.id}/added_to_queue", payload=invoice_title
                )
        except:
            logging.error("Exception when publishing queue add to mqtt")
            pass
            ...
            [
                [
                    InlineKeyboardButton(
                        ...
                        url=f"https://{settings.domain}/jukebox/payinvoice?payment_hash={invoice.payment_hash}",
                    ),
                    InlineKeyboardButton(
                        "Cancel",
                        callback_data=telegramhelper.add_command(
...
async def main() -> None:
    ...
    async def web_search(request: Request) -> JSONResponse:
        ...
        if chat_id is None:
            return JSONResponse(
                {"status": 400, "message": "Incomplete request. Chat ID is None"}
            )
        ...
        try:
            ...
            return JSONResponse(
                {"status": 400, "message": "Incomplete request. Failed to cast chat_id"}
            )
        ...
        if form is None:
            return JSONResponse(
                {"status": 400, "message": "Incomplete request form is None"}
            )
        ...
        if query is None:
            return JSONResponse(
                {"status": 400, "message": "Incomplete request query is None"}
            )

        # validate allow characters in query
        if not re.search("^[A-Za-z0-9 ]+$", query):
            return JSONResponse(
                {"status": 400, "message": "Incomplete request query is invalid"}
            )
        ...
        if auth_manager is None:
            return JSONResponse(
                {"status": 400, "message": "Incomplete request auth manager is None"}
            )
        ...
        if sp is None:
            return JSONResponse(
                {"status": 400, "message": "Incomplete response sp is none"}
            )
        ...
        while numtries > 0:
            try:
                ...
                if numtries == 0:
                    ...
                    return JSONResponse(
                        {"status": 400, "message": "Search currently unavailable"}
                    )
                logging.warning("Spotify returned and exception, retrying")
                continue
            break
    ...
    async def web_add(request: Request) -> JSONResponse:
        ...
        invoice.title = invoice_title
        invoice.recipient = recipient
        logging.info("recipient when creating invoice: " + recipient.to_json())
        ...
        return JSONResponse(
            {
                "status": 200,
                "payment_url": f"https://{settings.domain}/jukebox/payinvoice?payment_hash={invoice.payment_hash}",
            }
        )
